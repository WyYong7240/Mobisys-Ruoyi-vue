<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="deploymentList" border class="custom-k8s-table">
      <el-table-column label="Deployment 名称" prop="name" min-width="250" show-overflow-tooltip>
        <template #default="scope">
          <span style="font-weight: bold; color: #303133;">{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="命名空间" prop="namespace" width="160" align="center" />
      
      <el-table-column label="副本状态 (Ready/Total)" width="200" align="center">
        <template #default="scope">
          <el-tag 
            :type="scope.row.readyReplicas === scope.row.replicas ? 'success' : 'danger'" 
            effect="light" 
            size="large"
      ...
      <el-table-column label="可用副本 (Available)" prop="availableReplicas" width="160" align="center" />
      <el-table-column label="创建时间" prop="creationTimestamp" width="200" align="center" />
...
const getList = () => {
  loading.value = true
  listDeployments(queryParams.value).then(response => {
