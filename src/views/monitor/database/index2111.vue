      ...
      <el-col :span="20">
        <div class="right-container" ref="rightContainer">
          ...
          <el-card v-if="!dbInfo.name" shadow="never" class="mb20">
            ...
            <el-row :gutter="20">
              <el-col :span="8" v-for="dim in dimensionList" :key="dim.key">
          ...
          <template v-else>
            ...
            <el-card shadow="never" class="mb20">
              <template #header>
                MySQL 告警指南
                <span class="tips-icon" v-tooltip="'下面的PromQL语句可用于告警规则设置'">!</span>
              </template>
              <el-collapse>
                <el-collapse-item
                  v-for="alarm in alarmList"
                  :key="alarm.name"
                  :title="alarm.name + '（阈值：' + alarm.threshold + '）'"
                >
                  <div class="alarm-desc">{{ alarm.desc }}</div>
                  <el-input
                    type="textarea"
                    :rows="4"
                    :model-value="generatePromQL(alarm.promql)"
                    readonly
                  />
                  <div class="generate-form">
                    <el-select v-model="selectedTime[alarm.name]" placeholder="选择时间窗口" size="small" style="width: 120px;">
                      <el-option v-for="t in timeOptions" :key="t" :label="t" :value="t" />
                    </el-select>
                    <el-button size="small" @click="updatePromQL(alarm)">生成</el-button>
                  </div>
                  <div class="generated-promql">{{ generatedPromQL[alarm.name] }}</div>
                </el-collapse-item>
              </el-collapse>
            </el-card>
...
/* Grafana */
const grafanaBaseUrl = "http://192.168.31.34:32556/d-solo/549c2bf8936f7767ea6ac47c47b00f2a/mysql-exporter-quickstart-and-dashboard"
...
/* 面板 */
const panelGroup = {
  basic: [{ id: 12, name: "运行时长 " }, { id: 9, name: "网络流量监控 " }, { id: 50, name: "内存使用监控 " }],
  db: [
    { id: 13, name: "QPS 监控 " }, { id: 92, name: "连接数监控 " }, { id: 48, name: "慢查询监控 " },
    { id: 30, name: "排序操作监控 " }, { id: 22, name: "临时对象监控 " }, { id: 311, name: "全表扫描监控 " },
    { id: 10, name: "客户端线程活动监控 " }, { id: 53, name: "数据库查询负载监控 " }
  ],
  storage: [
    { id: 43, name: "文件打开监控 " }, { id: 41, name: "打开文件数监控 " }, { id: 44, name: "表打开缓存监控 " },
    { id: 42, name: "打开表数监控 " }, { id: 54, name: "表定义缓存监控 " }, { id: 50, name: "内存缓存使用监控 " }
  ],
  replication: [],
  exception: [{ id: 32, name: "表锁监控 " }, { id: 47, name: "异常连接监控 " }]
}

/* 告警指南 */
const alarmList = ref([
  { name: "缓存利用率", threshold: "90%", desc: "缓存占用过高，可能存在内存瓶颈", promql: "(mysql_global_status_innodb_page_size * mysql_global_status_buffer_pool_pages{state=\"data\"}) / mysql_global_variables_innodb_buffer_pool_size * 100" },
  { name: "锁等待比例", threshold: "10%", desc: "锁冲突严重", promql: "sum(rate(mysql_global_status_table_locks_waited[1m])) / (sum(rate(mysql_global_status_table_locks_immediate[1m])) + sum(rate(mysql_global_status_table_locks_waited[1m]))) * 100" },
  { name: "慢查询", threshold: "5", desc: "SQL性能问题", promql: "sum(rate(mysql_global_status_slow_queries[1m]))" },
  { name: "连接池使用率", threshold: "85%", desc: "连接接近上限", promql: "(mysql_global_status_threads_connected / mysql_global_variables_max_connections) * 100" },
  { name: "QPS过高", threshold: "100", desc: "系统负载过高", promql: "rate(mysql_global_status_queries[1m])" }
])
...
/* PromQL 动态生成 */
const timeOptions = ["1h", "10m", "5m", "1m"]
const selectedTime = reactive({})
const generatedPromQL = reactive({})

function generatePromQL(promql) {
  if (!dbInfo.ip || !dbInfo.job) return promql
  return promql.replace(/job=~".*?"/, `job=~"${dbInfo.user}"`).replace(/instance=~".*?"/, `instance=~"${dbInfo.ip}:${dbInfo.port}"`)
}

function updatePromQL(alarm) {
  const t = selectedTime[alarm.name] || "1m"
  generatedPromQL[alarm.name] = generatePromQL(alarm.promql).replace(/\[.*?\]/, `[${t}]`)
}
...
function handleNodeClick(data) {
  ...
  if (data.status === 1) { ElMessage.warning("该实例已禁用"); return }
...
.right-container { height: calc(100vh - 20px); overflow-y: auto; padding-right: 10px; }
...
.section-title { font-size: 18px; font-weight: bold; margin: 15px 0; border-left: 4px solid #409eff; padding-left: 8px; }
.iframe-container { border: 1px solid #ebeef5; border-radius: 6px; overflow: hidden; }
.iframe-header { background: #f8fafc; padding: 8px; font-weight: bold; border-bottom: 1px solid #ebeef5; }

.generate-form { margin-top: 8px; }
.generated-promql { margin-top: 8px; font-family: monospace; }
